...
def train_client_on_gpu(client, gpu_id, epochs, server_prototypes, round_num):
    ...
    if server_prototypes is not None:
        server_prototypes = {k: v.to(device) for k, v in server_prototypes.items()}
...
def parallel_train_clients(clients, epochs, server_prototypes, round_num):
    ...
    if not config.USE_MULTI_GPU or len(config.GPU_IDS) <= 1:
        ...
        for client in clients:
            ...
            client.device = device
            if server_prototypes is not None:
                sp = {k: v.to(device) for k, v in server_prototypes.items()}
            else:
                sp = None
            client.train(epochs=epochs, server_prototypes=sp, round_num=round_num)
    ...
    primary_device = torch.device(primary_gpu)
    for client in trained_clients:
        # Move to CPU to save VRAM on primary GPU
...
def fedmd_parallel_train_clients(clients, logits_matching_epochs, private_training_epochs,
    ...
    primary_device = torch.device(primary_gpu)
    for client in trained_clients:
        client.model = client.model.to(primary_device)
        client.device = primary_device
